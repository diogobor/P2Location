...
public class ExportPTMsAction extends AbstractCyAction {
	...
	private static final String MENU_CATEGORY = "Apps.P2Location.Post-translational Modifications";
	...
	public void actionPerformed(ActionEvent e) {
		...
		JOptionPane.showMessageDialog(null, msg, "P2Location - Export PTM(s)", JOptionPane.INFORMATION_MESSAGE,
		...
		try {
			...
			if (Util.ptmsMap.containsKey(myNetwork.toString())) {
				...
				if (taskMonitor == null) {
					JOptionPane.showMessageDialog(null, "File has been saved successfully!",
							"P2Location - Export PTM(s)", JOptionPane.INFORMATION_MESSAGE);
				} else {
				...
				if (taskMonitor == null) {
					JOptionPane.showMessageDialog(null, "Network has not been found!", "P2Location - Export PTM(s)",
			...
			if (taskMonitor == null) {
				String errorMsg = "<htmml><p>ERROR: It is not possible to save the file.</p><p>" + e.getMessage()
						+ "</p></html>";
				JOptionPane.showMessageDialog(null, errorMsg, "P2Location - Export PTM(s)", JOptionPane.ERROR_MESSAGE);
			} else {
